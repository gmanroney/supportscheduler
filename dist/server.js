...
router.route('/engineer/:engineer_id').get(function (req, res) {
...
router.route('/engineer/gender/:gender').get(function (req, res) {
...
router.route('/engineer/empid/:empid').get(function (req, res) {
...
router.route('/schedules/:schedule_year/:schedule_period').post(function (req, res) {
  ...
  query.exec(function (err, records) {
    ...
    for (var count in results) {
      ...
      Schedule.findOneAndUpdate({ ymd: results[count].ymd, shift: results[count].shift }, results[count], { upsert: true, new: true, runValidators: true }, function (err, doc) {
        if (err) res.send(err);
      });
    };
  });
  ...
  Schedule.find({ wn: { $in: [req.params.schedule_period, parseInt(req.params.schedule_period) + 1] } }, function (err, schedule) {
    if (err) {
      res.send(err);
    };
    res.json(schedule);
  ...
  Schedule.remove({ wn: { $in: [req.params.schedule_period, parseInt(req.params.schedule_period) + 1] } }, function (err, schedule) {
    if (err) res.send(err);
...
router.route('/schedule/:schedule_id').get(function (req, res) {
...
// get engineer based on employee ID
router.route('/schedule/empid/:empid').get(function (req, res) {
...
router.route('/schedule/date/:date').get(function (req, res) {
...
// on holiday dates.
router.route('/refdatas').post(function (req, res) {
  // create record
...
router.route('/refdata/:refdata_id').get(function (req, res) {
